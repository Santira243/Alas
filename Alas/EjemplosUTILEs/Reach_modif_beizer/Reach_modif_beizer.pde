...
//   reachSegment1(0, x[0], y[0]);
 //  reachSegment2(0, x[0], y[0]);
  ...
  for(int i=0; i<x2.length; i++) {
     segment(x2[i], y2[i], angle2[i],(i+2)*1.5, segLength2,0.5);
  }
  for(int i=0; i<x1.length; i++) {
     segment(x1[i], y1[i], angle1[i],(i+7)*1.5, segLength1,0.7);
  }

  for(int i=0; i<x.length; i++) {
    segment(x[i], y[i], angle[i]*0.3, (i+10)*1.5, segLength,1);
  }
...
void segment(float x, float y, float a, float sw, float sl, float sca) {
  strokeWeight(sw/2);
  for(int aux=0; aux < 2; aux++)
  {
  float mx = x + cos(a) * aux*sl/2;
  float my = y + sin(a) * aux*sl/2;
  pushMatrix();
  translate(mx, my);
  if(a<0)
  rotate(a);
